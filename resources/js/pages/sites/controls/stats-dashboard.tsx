...
export default function StatsDashboard({ node }: PanelControlProps) {
  const { server, site } = usePage<{ server: Server; site: Site }>().props;
  const hasStatsService = Boolean(node.props.hasStatsService);
  const statsEnabled = Boolean(node.props.statsEnabled);

  if (statsEnabled && hasStatsService) {
    return <StatsView server={server} site={site} />;
  }
    ...
    <>
      ...
      <Card>
        <CardContent className="text-muted-foreground p-6 text-sm">
          {!statsEnabled ? (
            <>
              Statistics are <span className="text-foreground font-medium">disabled</span> for this site. Enable them from the site's Settings page.
            </>
          ) : (
            <>
