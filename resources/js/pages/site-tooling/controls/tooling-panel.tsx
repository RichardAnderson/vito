...
export default function ToolingPanel({ node }: PanelControlProps) {
  const page = usePage<{ server: Server; site: Site }>();
  const site = useRealtimeRecord<Site>(page.props.site, 'site')!;

  const configs = useConfigs();
  const tools = configs?.tooling ?? [];
  const { isolated_user, sibling_sites, installed_versions, tool_statuses, required_tooling } = node.props as unknown as SiteToolingProps;
